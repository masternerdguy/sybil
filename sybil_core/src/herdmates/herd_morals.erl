...
start() ->
    ...
    %% register process
    register(herd_morals_process, PID),
...
dirty_read() ->
    herd_morals_process ! {self(), dirty_read}.

%% @doc Requests a clean read of the session.
clean_read() ->
    herd_morals_process ! {self(), clean_read}.
...
process_init() ->
    ...
    CN = ssh_helper:connect("172.20.128.5", "dubuntu", "secret_password"),
...
process(CN) ->
        ...
        {PID, clean_read} ->
            ssh_helper:wait_for_prompt(CN),
            PID ! {herd_morals_process, clean_read, ssh_helper:capture_tmux(CN)};
...
setup_query() ->
    "you are responsible for providing moral guidance to a larger entity. you are guided by Roman Catholicism, with an emphasis on Papal Encyclicals and the writings of the Doctors of the Church.".
